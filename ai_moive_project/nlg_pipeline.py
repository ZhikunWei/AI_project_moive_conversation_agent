...
class nlg_subsystem:
    ...
    def _director_of(self, director, movie):
        if isinstance(movie, str):
            movie = [movie]
        if len(director) == 1:
            return director[0] + ' is the director of movie(s) ' + self._wrap_multi_movies(movie)
        return ' '.join(director) + ' are the directors of movie(s) ' + self._wrap_multi_movies(movie)

    def _writter_of(self, writter, movie):
        if isinstance(movie, str):
            movie = [movie]
        if len(writter) == 1:
            return writter[0] + ' is the writter of movie(s) ' + self._wrap_multi_movies(movie)
        return ' '.join(writter) + ' are the writters of movie(s) ' + self._wrap_multi_movies(movie)

    def _actor_of(self, actor, movie):
        if isinstance(movie, str):
            movie = [movie]
        if len(actor) == 1:
            return actor[0] + ' is the actor/actress of movie(s) ' + self._wrap_multi_movies(movie)
        return ' '.join(actor) + ' are the actors/actresses of movie(s) ' + self._wrap_multi_movies(movie)
            ...
            if q[1].endswith('?'):
                ...
                if a[1] == 'genre_of':
                    if q[0] == None or (isinstance(a[0], list) and q[0] == 'LAST?'):
                        ...
                        answer = 'Movies ' + self._wrap_multi_movies(a[2]) + ' are also of type ' + a[0]
                if a[1] == 'writter_of':
                    answer = self._writter_of(a[0], a[2])
                    ...
                    if q[0] == None or (isinstance(a[0], list) and q[0] == 'LAST?'):
                        ...
                        answer = 'Movies ' + self._wrap_multi_movies(a[2]) + ' are also of rate ' + a[0]
                elif a[1] == 'year_of':
                    if q[0] == None or (isinstance(a[0], list) and q[0] == 'LAST?'):
                        answer = self._year_of(a[0], a[2])
                    else:
                        answer = 'Movies ' + self._wrap_multi_movies(a[2]) + ' are also of year ' + a[0]
                answer_list_str.append(answer)
