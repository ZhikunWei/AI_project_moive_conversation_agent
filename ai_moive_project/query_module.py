...
class query_extract_module:
    ...
    def _maybe_actor(self, sent):
        s = [x for x in sent]
        _, info = words_matching(s, ['acts', 'stars', 'star', 'act', 'actor', 'actress', 'actors', 'in'], '_', exact_match=False, word_to_vec=self.word_to_vec, ignore_words=self.stop_words)
        if len(info) > 0:
            return True
        return False
        ...
        if 'Who' in sentence or 'who' in sentence:
            ...
            if self._maybe_rate(sentence):
                ...
                if len(rate) == 0 and len(movie) == 0:
                    ...
                    return [('LAST', 'rate_of', movie[0])]
                elif len(movie) == 0:
                    return [(rate[0], 'rate_of', 'LAST')]
                else:
                    return None
            
            if self._maybe_genre(sentence):
                ...
                if len(gen) == 0 and len(movie) == 0:
                    ...
                    return [('LAST', 'genre_of', movie[0])]
                elif len(movie) == 0:
                    return [(gen[0], 'genre_of', 'LAST')]
                else:
                    return None
            
            if self._maybe_year(sentence):
                _, year = detect_year(sentence)
                if len(year) == 0 and len(movie) == 0:
                    return [('LAST?', 'year_of', 'LAST?')]
                elif len(year) == 0:
                    return [('LAST', 'year_of', movie[0])]
                elif len(movie) == 0:
                    return [(year[0], 'year_of', 'LAST')]
                else:
                    return None
        else:
            # true/false question
...
if __name__ == '__main__':
    qp = query_extract_module(get_word2vec())
    x = input()
    while x != 'q':
        token = tokenize(x)
